        ...
        protected static async Task<int> HandleCommand(ParseResult parseResult, CommandArgs cliArgs)
        {
            ...
            using (var consoleContext = Console.WithColor(ConsoleColor.Cyan))
            {
                ...
                if (loadResult.Model1Type == typeof(Models.DbSchema.DatabaseSchema))
                {
                    ...
                    await logger.WriteLineAsync(ConsoleColor.Cyan, $"For a sample schema please check out: '{ConsoleColor.White}https://github.com/Drizin/CodegenCS/blob/master/src/Models/CodegenCS.Models.DbSchema.SampleDatabases/AdventureWorksSchema.json{PREVIOUS_COLOR}'");
                }
