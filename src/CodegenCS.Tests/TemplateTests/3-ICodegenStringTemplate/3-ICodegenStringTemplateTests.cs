...
{
    public FormattableString GetTemplate(Table table)
    {
...
{
    public ICodegenStringTemplateTests() : base(GetCurrentFolder()) { }
...
class MyPocoTemplate3 : ICodegenStringTemplate<DatabaseSchema>
{
    ...
    public FormattableString GetTemplate(DatabaseSchema schema) => $$"""
                    ...
                    {
                        {{ schema.Tables.Select(t => GenerateTable(t)) }}
                    }
                    """;
    ...
    FormattableString GenerateTable(Table table) => $$"""
                    ...
                    public class {{ table.TableName }}
                    {
                        {{ table.Columns.Select(c => GenerateColumn(table, c)) }}
                    }
                    """;

    FormattableString GenerateColumn(Table table, Column column) => $$"""
            /// <summary>
...
partial class ICodegenStringTemplateTests : BaseTest
{
    ...
    public void Test32()
    {
        ...
        AssertContentIsEqualToFile(writer.GetContents(), "MyDatabase.cs");
    }
}
