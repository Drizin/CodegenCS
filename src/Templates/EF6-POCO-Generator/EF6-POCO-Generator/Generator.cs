...
public class Generator
{
    ...
    public void GenerateMultipleFiles(string outputFolder, string csproj = null)
    {
        ...
        MSBuildProjectEditor editor = null;

        if (csproj != null)
        {
            // since no errors, first modify csproj, then we save all files
            editor = new MSBuildProjectEditor(csproj);
            foreach (var o in _context.OutputFilesAbsolute(outputFolder))
                editor.AddItem(itemPath: o.Key, itemType: o.Value.FileType);
            editor.RemoveUnusedItems(outputFolder); // remove from csproj all files under GeneratedCode except those which were added by AddItem above
        }

        _context.SaveFiles(outputFolder);
    ...
    public void GenerateSingleFile(string file)
    {
        ...
        _context.SaveFiles(); // since the file path was defined using absolute path, we don't have to specify base folder
    }
